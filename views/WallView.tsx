...
const WallView: React.FC<WallViewProps> = ({ participants, showVotes = false }) => {
  // 計算總票數
        ...
        <div className="relative flex-1 overflow-hidden rounded-3xl bg-slate-900/20 border border-slate-800/50">
          ...
          >
            {displayParticipants.map((p, index) => (
              <div 
                key={`${p.id}-${index}`} 
                className="transform-gpu transition-transform duration-300 hover:scale-[1.02] hover:z-10"
              >
